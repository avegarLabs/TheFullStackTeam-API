...
namespace TheFullStackTeam.API.Controllers.Admin
{
    ...
    public class ProfessionalController: AdminBaseController
    {
        [HttpGet]
        ...
        /// </summary>
        /// <returns></returns>
        [HttpGet("cv/{ProfessionalId}")]
        [ProducesResponseType(typeof(ProfessionalListItem), StatusCodes.Status200OK)]
        public async Task<IActionResult> GetCVPdf(Guid ProfessionalId)
        {
            var queryResponse = await Mediator.Send(new GetProfessionalCvPdfQuery(ProfessionalId));
            if (queryResponse.Success)
            {
                return Ok(queryResponse.Data);
            }
        ...
        [HttpGet("findJob/{ProfessionalId}")]
        [ProducesResponseType(typeof(JobSugestionsListItem), StatusCodes.Status200OK)]
        public async Task<IActionResult> GetListJob(Guid ProfessionalId)
        {
            var queryResponse = await Mediator.Send(new ListJobsSugestionsQuery(ProfessionalId));
            if (queryResponse.Success)
            {
                return Ok(queryResponse.Data);
            }
        ...
        /// <returns></returns>
        [HttpPut("{ProfessionalId}/projects/{ProjectId}")]
        [ProducesResponseType(typeof(ProjectListItem), StatusCodes.Status200OK)]
        ...
        public async Task<IActionResult> UpdateServicesAddSkill(Guid ProfessionalId, Guid ServiceId, [FromBody] SkillModel skill)
        {
            var response = await Mediator.Send(new AddSkillToProfessionalServicesCommand(ProfessionalId, ServiceId, skill));
            if (response.Success)
            {
                return Ok(response.Data);
            }
        ...
        /// <summary>
        /// Update a professional Languege
        /// </summary>
        /// <param name="model">ProfessionalLanguegeListItem model</param>
        /// <param name="ProfessionalId">Professional Id</param>
        /// <param name="ProfessionalLanguegeId">Professional Languege Id</param>
        /// <response code="200">Item updated success</response>
        /// <returns></returns>
